...
public class BytePacker {
    ...
    /**
     * Packs a float in little-endian into a byte array.
     *
     * @return a byte array of length Float.BYTES
     */
    ...
    /**
     * Packs a long in little-endian into a byte array.
     *
     * @return a byte array of length Long.BYTES
     */
    ...
    /**
     * Packs an int[] as WPILOG int64[] (8 bytes per element, little-endian)
     * by widening each element to long.
     *
     * @param values the int values to pack
     * @return a byte array of length values.length * Long.BYTES
     */
    public static byte[] packIntsAsLongs(int[] values) {
        ByteBuffer buf = ByteBuffer
            .allocate(values.length * Long.BYTES)
            .order(ByteOrder.LITTLE_ENDIAN);
        for (int v : values) {
    ...
    /**
     * Packs an Integer[] as WPILOG int64[] (8 bytes per element, little-endian)
     * by widening each element to long.
     *
     * @param values the Integer values to pack
     * @return a byte array of length values.length * Long.BYTES
     */
    public static byte[] packIntsAsLongs(Integer[] values) {
        ByteBuffer buf = ByteBuffer
            .allocate(values.length * Long.BYTES)
            .order(ByteOrder.LITTLE_ENDIAN);
        for (int v : values) {
     ...
     *
     * @return a byte array of length 1
     */
    ...
    /**
     * Packs a String as UTF-8 bytes for a WPILOG "string" entry.
     */
     ...
     * 4-byte LE count N, then for each string: 4-byte LE length followed by UTF-8 bytes.
     */
